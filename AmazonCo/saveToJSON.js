...
async function main() {
    ...
    for (let brand of newBrands) {
        ...
        try {
            console.log(info(`(i) Saving ${brand} to JSON File...\n`));
            await fs.writeFile('totals.json', JSON.stringify(brandsWithTotal), (err) => {
                if (err) {
                    console.log(error(`(⬣) Failed to save ${brand} to JSON File\n`));
                } else {
                    console.log(success(`(✓) Saved ${brand} to JSON File Successfully!\n`));
                }
            });
        } catch (err) {
            console.log(error(`(⬣) Failed to save ${brand} to JSON File\n`));
